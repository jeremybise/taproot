...
**The long form of the rules in [CLAUDE.md](CLAUDE.md).** That file is loaded into every session and
states each constraint plus a one-line reason. This one is not loaded automatically; it records the
part that does not fit there — what was measured, what was tried and rejected, and how each rule was
originally got wrong.

**Read the relevant entry here before changing, relaxing, or "simplifying" a rule in CLAUDE.md.**
Most of these look like arbitrary fussiness until you know the failure behind them, and nearly all of
them were written after that failure had already shipped. The two files describe the same rules and
must not disagree: if you change a constraint, change it in both.

Nothing here is a plan. Anything not yet built lives in [SCOPE.md](SCOPE.md).
...
**`reorderSiblings` is the write path `position` never had, and it takes a whole level.**
`createItem` wrote `siblings.length` once and nothing ever wrote the column again, so the order a
consumer is handed a page's children in — which is what a site's sub-navigation and any "pages in this
section" listing come out in — was permanently the order somebody happened to create them in. The only
repair available to an editor was to delete a page and make it again, losing its revisions, its id and
every link pointing at it. It survived because every ordering test built its fixture through
`createItem`, so creation order and intended order were the same string in every assertion. Four things
shape the function:

- **A whole level, and an exact one.** Positions are `0..n-1`, so a *partial* level hands its rows
  positions that others already hold and leaves duplicates ordered by the `title` tiebreak — which
  presents as dragging working on some rows and not others. Exactness also settles concurrency for free:
  two editors on one section, one inserting while the other drags, and the stale list is refused rather
  than shuffling a row into a level it was never part of.
- **Reordering never re-parents.** Reordering writes one integer per row, while re-parenting rewrites the
  item's path, every descendant's path, and a redirect for each. Keeping them apart is why this is its own
  function rather than a key on `UpdateItemInput` — a drag landing a few pixels too far right would
  otherwise silently rewrite forty URLs.
- **The parent's `updated_at` moves too.** Its delivery response carries these children in this order and
  `getItemVersionByPath` answers a conditional request from `updated_at` alone, so leaving the parent's
  stamp still means a cached copy revalidating, being told 304, and having its freshness renewed against
  an order that changed — unbounded under RFC 9111 §4.3.4 rather than capped at the TTL, the trap
  `reusableBlockLibraryVersion` closes one feature along. A no-op reorder writes nothing, for the same
  reason in reverse.
- **No revision.** Revisions snapshot `title`, `slug`, `status`, `data` and `seo`; position is in none of
  them, so one written here would record nothing and restore nothing.

There is currently no admin screen for it — the only UI that ever drove it was the book outline's
drag-and-drop, which went with books. `POST /api/taproot/items/{id}/children/reorder` is the only
caller, and root-level items are unreachable through it because there is no parent id to put in the path,
though `reorderSiblings` itself accepts `null`. **If a screen is built for it, drag-and-drop needs
keyboard controls beside it**, and each sibling group wants its own drop context so there is nowhere for
a row to land but among its own level.

One lesson from the screen that did exist is worth keeping even though the screen is gone: it conveyed
depth with `padding-inline-start` in a flat table, which is fine to read and impossible to drag — a flat
list across five levels contains no sibling group. Converting it to a nested list silently broke
`a11y-audit.mjs`, which had been picking the richest route by counting `<tr>`: every candidate then
scored zero, so the *first* one won, which is exactly the selection the count exists to avoid. **A
heuristic that greps rendered markup goes stale silently, towards auditing the wrong route while still
reporting a clean run.**

**`content_types.hide_from_nav` keeps a type out of the sidebar and out of nothing else.** The sidebar is
built from the content types that exist, one entry each, which is right up to about a dozen and wrong past
it: a `person` in a staff directory or a `policy` reached only through the page that lists it is real
content — versioned, classified, indexed — that nobody ever navigates to *by type*, and entries nobody
clicks push the ones people use every day below the fold.

The interesting part is what it must **not** be used for. Read it through `isNavigable`, and never filter
a listing by it: that would be a delete that does not delete, stranding every item of the type in a
deployment whose own UI could not reach them. It is the cheapest possible place to break the "never leave
a deployment in a state its own UI cannot reach" rule — one extra `.filter` on a query, in a file nobody
would think to check, with no error and no visible failure until somebody goes looking for a page they
wrote. A hidden type keeps its list screen, its create screen, its items, and its place in **All content**
and in search; `navVisibility.test.ts` asserts all four of those negatives rather than the positive.

Two smaller decisions: blocks are excluded because a block type has no list screen to link to, so a
caller passing `includeBlocks` and then building a sidebar would get entries that 404; and the column is
meaningful for **every** kind, unlike `url_prefix` or `preview_path`, because a singleton is exactly as
clutterable as a collection — so neither write path forces it by kind, and the settings checkbox renders
for every kind that has a sidebar entry.

**A repeater can keep its own rows in order, and it is off by default because order is often the
meaning.** `sortBy`/`sortDirection` live on the field's config and are applied **on write** inside
`validateRepeater`, so stored order is delivered order, delivery pays nothing, and the payload still
round-trips into a write. The case is a long list whose order is by some key and always was — a glossary
by term, course codes — where a drag does not help, because placing one row of forty is miserable however
smooth the interaction. Four things:

- **It compares with `Intl.Collator({ numeric: true })`**, or `RAD 1096` sorts before `RAD 196` — wrong in
  the way that looks plausible on every example anybody checks by hand.
- **A row missing the key sorts last in both directions**, so reversing does not float half-typed rows to
  the top.
- **The sort is stable**, so equal keys keep their entered order.
- **A key naming a sub-field that no longer exists does nothing**, the rule a query field's
  `dateFieldApiId` already follows: a stored preference outlives what it names, and a live page must not
  break for a configuration mistake made weeks earlier on another screen.

Not the default, because plenty of repeaters carry their meaning *in* the order they were typed — an
event's schedule, the steps of a process, a program plan where a blank `term` means "same as the row
above". Nothing can detect that, so it has to be asked.
...
of those nested inside a block or a repeater row all pointing back into the source subtree — none
