...
**5E — media multi-upload and bulk alt text** *(complete)*. One POST carrying several files, with an explicit cap on count and bytes, which is what keeps the plain server-rendered form working where N client-side POSTs would fork the no-JS path. The bulk describe screen honours all three alt states — blank is `null`, the Decorative checkbox is `''` — because a grid where blank means "skip" either marks images decorative by omission or wipes existing descriptions. It is reachable from the accessibility report, which linked undescribed images out one at a time; that is the higher-value half and was not in the original ask.

Two caps rather than one, and they behave differently on purpose: the count and total-bytes caps refuse the **whole request**, because a cap describes the request and no subset of it is valid — keeping the first ten of thirty is silent truncation with somebody's files in it. A single oversized file among nine good ones is the opposite, and sinking the batch there would cost the editor every other file, since a browser cannot reselect a partial file list. Those are collected and named on the next screen.

Three things fell out that the plan did not have. The upload form's **single alt box had to go**: one description cannot serve twenty images, so a box beside a `multiple` input is a control that silently applies to the first file or to none. The question moved to the screen immediately after — every upload lands on the describe grid, carrying the ids just written rather than re-querying for undescribed images, or three files uploaded into a library holding fifty would open on fifty-three strangers. The picker's inline box stays, because that one genuinely is a single image chosen for a single field.

The predicate is **shared with the report** (`MediaFilters.undescribed`), since a grid that disagreed with the count which sent somebody to it is the faceted-count bug one feature along.

And the defect worth recording: `AdminLayout`'s required `user` prop was omitted and a non-existent `activeNav` passed instead, which **typecheck cannot see** — the ambient `.astro` shim resolves the import without checking the file's contents — so the screen compiled, answered 200, and rendered a page whose entire body was missing. Found by loading it. Same blind spot as string arithmetic inside a component, and the reason the standing rule is that building is not evaluating.

**5F — menus: link chooser, new tab, nofollow** *(complete)*. `open_in_new_tab` existed end to end from `0004_menus` and had simply never been given a control; `no_follow` is new (`0031`); both are now checkboxes on the three add forms and on every row. The `term` panel turned out to be **already built** — the plan counted three target forms and there were three *including* term, so the "fourth panel" was a miscount rather than missing work.

`ALLOWED_REL` moved to its own importless `rel.ts` alongside `NEW_TAB_REL` and `menuRel`, and `sanitizeHtml.ts` imports rather than copies. What that bought is bigger than tidiness: a menu entry stored a bare `open_in_new_tab` and left the markup to the consumer, which makes protecting the visitor a rule every site has to know — and **both consumers that exist wrote `rel="noopener"` with no `noreferrer`**, independently. Two implementations making the same omission is the evidence that the composition belongs in core, so `menuRel` builds the string and it travels in the payload beside the flags, all the way through `MenuLink`. `noopener noreferrer` deliberately has no column and no checkbox: it is not a decision, and a control for it is a control somebody can untick.

Two things fell out. `rel` had a documented rule — the sanitiser's "added whatever the author asked for" — and **no test on either side**, in the file whose whole job is being the security boundary; it was true by luck rather than by evidence, and there is now a test asserting the two call sites compose the same answer, verified by mutation. And a checkbox read on a **patch** needs a presence marker, because absent means either "unticked" or "this form does not offer the control" — `openInNewTab: form.get(…) !== null` sat on the patch route for a phase, latent only because nothing rendered a form against it.

What shipped early is the defect underneath it. The item `<select>` was capped at 200, so a site with 300 pages had ~100 that could not be put in a menu at all — and which 200 you got was whatever path sorted first, so it was not even the top-level structure the cap's own comment appealed to. `ItemPicker` replaced it and the parent picker's 500 together, since `parentOptions.ts` had already predicted the control in its own comment; both limits now size a **first page** rather than a ceiling, with `total` reported so the control can say what it is not showing.

**The documented decision this band expected to override was not overridden, and that is worth recording as loudly as the override would have been.** The plan said menus work with JavaScript off today and would not afterwards. They still do: `ItemPicker` server-renders a plain `<select>` and upgrades on hydration, so the no-JS path is intact and the cap is gone. The sacrifice was assumed from the shape of the solution — a `LinkDialog` with four panels genuinely cannot degrade — rather than from the requirement, which was only ever "find a page without scrolling". A cheaper control met it. The remaining half of 5F may yet force the trade; it has simply not been paid yet, and should not be pre-paid.

**5G — AI assist** *(complete)*. Provider keys come from the **environment**, with Settings reporting set/not-set and never the value, exactly as the system screen already does for the cron secret; the settings row holds provider, model and feature toggles. That keeps the repo's "no encryption at rest anywhere" true rather than introducing AES and a new required secret. Every affordance is gated on an `available` flag, the way the mailer's `delivers` gates the forgot-password link — a Generate button that 500s because nobody set a key is the same failure as a form whose success message is a lie.

Thin `fetch`-based adapters, no SDKs — three of them (Anthropic, OpenAI, Gemini). That sits in tension with the mailer's "no vendor SDK or per-provider adapter" rule and resolves the other way for a stated reason: mail had a generic shape available and AI does not, and a webhook indirection here would be strictly worse than pasting a key. Alt text needs the image **bytes**, read through the storage adapter — not a URL, which may be unreachable from the Worker. Nothing generated is ever written straight to the row: it fills the input for a human to accept, because a machine writing `''` would silently mark an image decorative, which is the one thing the three-state rule exists to prevent.

Three things came out of building it. The SEO input is **`content_item_text`**, not a fresh walk over `data` — that row already holds the item's prose with blocks and repeater rows flattened, rebuilt in the item's write batch, so what the model reads is exactly what search matches and there is no second walk to drift from the first. A **missing** row is reported as "not indexed yet" rather than summarised as empty, which is the state a database sits in before `npm run db:reindex`. And an Anthropic **refusal is a 200** with empty content, so `stop_reason` has to be read before `content` — indexing `content[0].text` yields `undefined`, and a caller that stored it would mark the image decorative, which is the exact failure the whole no-write rule exists to prevent.

The describe grid's buttons are a **progressive enhancement over the server-rendered form**, not a React rewrite of it: the island renders only the buttons and fills the inputs the server already emitted, by id. Rewriting the rows in React would have meant reimplementing the three-state semantics inside an island and keeping a second no-JS copy of all of it. "Suggest for empty rows" runs **sequentially** — twenty parallel requests is twenty images at a provider that rate-limits, where the first 429 fails most of the batch — and skips any row a person has already written in.
...
**Phase 5.8 — Reuse, and the admin's own usability** *(complete)*
Unplanned, and the reason it exists is the reason Phase 5 exists: things noticed from using the CMS, except that this band was noticed from building a **real site** on it rather than from browsing the seed. A demo has six pages and one editor; a college has hundreds of each, and most of what follows is invisible until it does.

Three features, each answering "this value is written down in more than one place and the copies disagree":
- **Snippets** — a value defined once and substituted into prose everywhere by a `{{ tuition }}` token, plus a `snippet` field so a chart and a sentence can plot and read the same number. Modelled deliberately on `reusableBlocks.ts`, which solves the same problem one size up: a reusable block owns a region of a page, a snippet owns a value inside a sentence. A **typeable** token rather than rich text's opaque `taproot:item:{uuid}` marker, because it has to go in the middle of a sentence in a plain text input — which costs one thing, and is why `api_id` is immutable: the token *is* the reference. An unknown token is left exactly as written rather than blanked, since "Tuition is  per year" is a silent deletion that still looks plausible, and leaving it visible also means ordinary prose containing braces needs no escape syntax.
- **Summary templates** — replacing `content_types.title_field`, which was stored, validated, round-tripped through the API and **read by no admin list ever**: the same shape as `reverseLabel` and the old `needsPublish` flag, and the third time that pattern has been found here. A template rather than a second column beside it, because `title_field: 'headline'` *is* `{{ headline }}`. What it buys over a single field is the case a single field cannot express — a block instance has no title, so a collapsed row could only ever say its *type*, three "Card"s in a row telling an editor nothing about which card is which.
- **Configurable list columns** — every list rendered the same five columns, which is right for a page and says nothing about an event or a person. The default is preserved exactly rather than trimmed to something tidier, because this ships to deployments whose lists people are used to reading.

Then the usability pass proper: the searchable `ItemPicker` (see 5F), adding a block behind one control instead of a shouting library, a block's rarer actions behind an `OverflowMenu` with ordering kept on the row, an icon per content type, asking what kind of field it is **before** asking what to call it, and a sidebar/Settings mark to navigate by.

**Every capped list now either pages or says it is capped**, and the split is by what the screen is for. Several screens rendered a `total` counted before the limit in a header above a truncated list — the media library read "107 assets" over a grid of 100. The number was right, the rows were right, and together they said something false. `Pager` where an editor has to *reach* the rows (media, because a library outgrows a hundred assets as a matter of course; the audit log, because it grows with the deployment's age rather than its content and is the one table certain to exceed any cap), `TruncationNotice` where they have to *find* one and the search box above it is the answer. A cap that **is** the feature gets neither — Settings → Search asking for the top 50 terms is a leaderboard, and a note calling that a shortcoming would be describing the point of the screen.

**Phase 5.9 — Image delivery repair** *(complete)*
Four bugs in the pipeline 5.5 built, every one of them the same shape: correct picture, wrong weight, nothing thrown, nothing logged where a page view could show it, and **only weighing bytes finds any of them**. That shape is now the phase's main lesson, because `OUTPUT_QUALITY` had already taught it once and it was not generalised.

- **An output must always name a format.** The binding *rejects* an `output()` carrying no `format`, and `resizeImage` catches every failure and serves the stored bytes — so every variant asking only to be resized answered the full-size original. Measured on production: `?w=320` returned 170 KB of untouched JPEG where `?w=320&f=jpeg` returned 8.7 KB. The fake in `images.test.ts` accepted a format-less output, which is exactly why a suite already containing a width-only test stayed green throughout; it now refuses one the way the real binding does, the same lesson as `d1.test.ts` refusing PRAGMA.
- **The ladder must not offer a ceiling rung above its top rung.** A `w` descriptor is a promise about the delivered file's width, kept by snapping up and letting `fit: 'scale-down'` clamp back to the source — but above the ladder there is nothing left to clamp, so a 2000px source offered a `2000w` candidate capped to 1920: byte-identical to the `1920w` beside it, a second cache entry, and a width the browser sized its choice against wrongly. 62 of one real library's 107 images sat above the ladder. The suite reached `variantWidthsFor(1920)` and stopped, which is why the whole above-the-ladder case went unexamined.
- **`immutable` was a lie on a `?ar=` variant.** The header is justified by the storage key containing the asset's id — true of the original, false of a server-cropped variant whose rectangle is resolved from `hotspot_*` and `crop_*`, columns an editor changes from a screen built for changing them. There was **no remediation at all**: the route emits no `cache-tag`, and no purge reaches a browser cache anyway. `cropStamp` moves the *address* instead of shortening the lie.
- **Media writes purged nothing**, alone among entities. Editing a hotspot, a crop or alt text updated the row and cleared no cache, so the delivery JSON kept the old value for its full day and the consumer's HTML was never flushed. It presents as "my edit did not save", which sends you to the write path — the one part that was working.

Two additions rather than repairs: `TaprootImage` gained a **ratio-less** mode, because a page-title band and a call-to-action are `min-h` plus however tall their copy turns out, so both had reached for a plain `<img>` to escape the ratio and given up the resize on the way out — skipping the crop was never a reason to skip the resize. And AVIF ships through `<picture>`, which is **not** the `f=auto` this repo rejects: each `<source>` names one format in its own URL, so each is its own cache key and the browser chooses before it makes a request at all.

**Phase 6 — Integrations** *(under way)*
...
**6A — webhooks** *(complete)*. Six events — `item.created`, `item.updated`, `item.published`, `item.unpublished`, `item.deleted`, `release.published` — signed per endpoint, retried by the existing sweep, and logged where somebody can read the reason. Five things settled while building it, each with a plausible wrong answer:

- **Enqueue, then attempt — the opposite of the purge queue, and the one place the two genuinely differ.** A purge attempts first and records only failures, which is right because losing one costs staleness the TTL already bounds. An event is a fact about a moment that nothing regenerates, so the row is committed *before* the request leaves and an isolate killed mid-send leaves work the sweep finds rather than an event that never existed. `0023_pending_purges` invited exactly this reconsideration when a second kind of deferred work appeared; reconsidered, and they stay separate — different durability, different columns, and one of them is read by a screen.
- **The event says what changed and never carries the content.** A payload with fields would be a second read contract with no key, no scope check and no visibility rules, arriving at an admin-typed URL. Keeping it a notification means a receiver gets published content by construction, the payload does not track the content model, and an endpoint cannot be used to pull out drafts.
- **The signing secret is the first secret stored recoverable, and that is not a reversal of the AI-key rule.** An HMAC has to be *produced*, not compared, so a hash cannot sign. What made a provider key inadmissible was that it is a credential on somebody's paid account; this one is minted by Taproot, authenticates a message rather than authorising access, and rotates in one button. The alternative — one master secret in the environment, per-endpoint keys derived from it — is closed by the precedent that killed signed preview tokens: it needs a working default for `npm run dev`, and a default signing secret is not a secret.
- **The timestamp goes inside the signed message.** Signing the body alone and sending `t=` beside it is the version that looks identical and is worthless, since a captured request replays with a fresh stamp. Proven by mutation: making both halves sign the body alone fails exactly one test.
- **`item.updated` and `item.published` both fire for one save, and that is not two spellings of one fact.** They answer different questions and an endpoint subscribes to whichever it acts on. Publication is judged by crossing the boundary rather than by the destination status, which is why `published → archived` is an unpublish — a call site checking `status === 'published'` gets it wrong, and `canChangeStatus` already exists because that mistake was made three times over.

Two things fell out. `API_KEY_SCOPES` had already stopped being a one-element array when the search log added `search:write`, so the note this section used to carry about expanding it is spent. And **restoring a revision declared no cache tags at all** — a write that changes a live page's title, body and status, invalidating nothing, which is the media-write bug of 5.9 one route along.

**MCP server exposure** belongs here rather than on the radar: let AI agents query/create/update content through an MCP server rather than raw REST. The visual content-type builder already gives every content type an introspectable typed schema, and API keys already provide an auth/scoping mechanism that doubles as agent-access control. It needs the first scope that is a *content* write, which is a bigger decision than `search:write` was — an appended log row cannot damage anything and an item write can.
